...
function build_one
{
./configure --prefix=$PREFIX $COMMON $CONFIGURATION --enable-gpl --enable-libx264 --cross-prefix=$TOOLCHAIN/bin/aarch64-linux-android- --target-os=linux --arch=aarch64 --enable-cross-compile --sysroot=$SYSROOT --extra-cflags="-O3 -DANDROID -Dipv6mr_interface=ipv6mr_ifindex -fasm -Wno-psabi -fno-short-enums -fno-strict-aliasing $ADDI_CFLAGS" --extra-ldflags="-Wl,-rpath-link=$SYSROOT/usr/lib -L$SYSROOT/usr/lib -nostdlib -lc -lm -ldl -llog $ADDI_LDFLAGS"
